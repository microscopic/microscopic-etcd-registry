...
class ETCDServiceRegistry {
  ...
  getServiceOptions (serviceName) {
    return this._getServiceOptions(5, serviceName)
  }
   ...
   * @protected
   */
  _getServiceOptions (maxRetries, name) {
    try {
      const key = path.join(SERVICES_OPTIONS_KEY, name)

      const result = this[ _etcd ].getSync(key)
      ...
      /* istanbul ignore next */
      if (!result.body || !result.body.node) {
        throw new Error('Not found options service')
      }

      return JsonUtils.parse(result.body.node.value)
    } catch (error) {
      if (maxRetries <= 0) {
        return undefined
      }

      return this._getServiceOptions(maxRetries - 1, name)
    }
  }
}
